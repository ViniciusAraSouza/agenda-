...
class Tarefa:
    """
    Classe para representar uma tarefa, com métodos para salvar, obter, excluir 
    e atualizar tarefas em um banco de dados usando a classe `Database`.
    """
    ...
    def obter_tarefas(cls) -> list[Self]:
        with Database() as db:
            query: str = """
                        SELECT titulo_tarefa, data_conclusao, id FROM tarefas;
                         """
            resultados: list[Any] = db.buscar_tudo(query)
            tarefas: list[Self] = [
                cls(titulo, data, id) for titulo, data, id in resultados
            ]
            return tarefas
    ...
    def atualizar_tarefa(self: Self):
        with Database() as db:
            query: str = (
                """
                UPDATE tarefas SET titulo_tarefa = ?,
